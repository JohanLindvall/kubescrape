...
The agent talks only to the metadata service (`metaclient`) and an OTLP gRPC collector (`otlpexport`, built on `go.opentelemetry.io/collector/pdata`) — no Kubernetes API, no RBAC. Resource-attribute mapping lives in `agent/attrs`.

- **Tailer** (`agent/tailer`): single sweep goroutine over `/var/log/containers`; CRI parsing/partial reassembly in `agent/crilog`; app-level multi-line joining via `github.com/JohanLindvall/multiline` (per-file aggregator). At-least-once invariants: exports happen inline in the sweep with retries; per-file offsets are committed only after a successful export and never past entries still buffered in the multiline aggregator (`watermark`); on failure files are rewound to the committed offset; committed offsets are checkpointed to disk. A file's metadata is resolved (with server-side wait) before its data is consumed — nothing is read until it can be attributed.
- **Scraper** (`agent/promscrape`): hand-written streaming parser for the Prometheus text format (constant memory; do NOT replace it with `expfmt`, which buffers whole metric families — the 100k-series requirement is why it exists). Samples convert flat: counter/`_bucket`/`_sum`/`_count` → cumulative monotonic Sum, everything else → Gauge; no histogram grouping by design. Conversion chunks at `BatchPoints` data points per export so a scrape never resides in memory.

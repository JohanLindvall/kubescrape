...
The agent talks only to the metadata service (`metaclient`) and an OTLP gRPC collector (`otlpexport`, built on `go.opentelemetry.io/collector/pdata`) — no Kubernetes API, no RBAC (except `nodes/metrics` for the kubelet scrapes). Resource-attribute mapping lives in `agent/attrs`; `attrs.Filter` (`-resource-attrs-enable`/`-disable`, anchored regexes; nil = keep all) is applied at every point a final resource is built — tailer metadata resolution, both single-resource batchers, and the cadvisor per-pod resources. New resource-producing code paths must call `AttrFilter.Apply`.

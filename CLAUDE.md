...
- **Log enrichment** (`agent/logenrich`, flags `-logs-enrich`/`-journald-enrich`, both default true): `logenrich.Apply(lr, line)` runs `github.com/JohanLindvall/enrich` over each exported line in the tailer and journald `convert`/`flush` paths. Parsed timestamp overrides the CRI/journal time (ingest time stays in ObservedTimestamp), explicit level overrides severity (enrich's severity numbers ARE the OTLP numbers — cast directly), 32/16-hex IDs (dashes stripped, all-zero rejected) fill the trace/span fields, the rest become record attributes (`log.template`, `exception.*`, `azure.*`, `cloud.resource_id`). Body is never modified. New log-exporting paths should call `Apply` behind a config toggle.

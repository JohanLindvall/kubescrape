...
func TestMonitorNamespaceGate(t *testing.T) {
	...
	if !monitorAllowed(nil, newMonitorRefusals(), "servicemonitor", mon("team-a"), slog.New(slog.DiscardHandler)) {
		t.Error("nil set must allow everything (backward compatible default)")
	}
	if !monitorAllowed(set, newMonitorRefusals(), "servicemonitor", mon("monitoring"), slog.New(slog.DiscardHandler)) {
		t.Error("listed namespace must be allowed")
	}
	if monitorAllowed(set, newMonitorRefusals(), "servicemonitor", mon("team-a"), slog.New(slog.DiscardHandler)) {

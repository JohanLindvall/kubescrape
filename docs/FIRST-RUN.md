...
| `time() - kubescrape_informer_last_event_timestamp_seconds{resource="pods"}` large, on a cluster whose pods change | 15m | An established watch that has stopped delivering, which `kubescrape_apiserver_reachable` cannot see (a NEW connection still succeeds). Ambiguous on a quiet cluster — read it beside the probe. |

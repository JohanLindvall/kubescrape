...
| `kubescrape_transform_dropped_total` | `signal` | Records a transform script called drop() on, by signal: log records, metric data points (a dropped metric counts all of its points) and spans. Counted per export invocation: a producer retrying a transiently failed export re-runs the script and re-counts its drops. |

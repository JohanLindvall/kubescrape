//go:build race

package otlpingest

// raceEnabled reports whether this binary was built with -race. The race
// detector changes escape analysis and adds bookkeeping allocations, so
// testing.AllocsPerRun ceilings and heap measurements are meaningless under it
// — the budget tests skip rather than being widened to a number that would let
// a real regression through.
const raceEnabled = true

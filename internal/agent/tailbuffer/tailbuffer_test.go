...
func TestDecisionCacheEvictsUnderItsSizeCap(t *testing.T) {
	...
	c.put(traceID(2), false, now)
	c.put(traceID(3), true, now)
...
func TestDecisionCacheHandlesARedecidedTrace(t *testing.T) {
	...
	now := time.Unix(1, 0)
	c.put(traceID(1), true, now)
	c.put(traceID(1), false, now) // re-decided
	c.put(traceID(2), true, now)
	c.put(traceID(3), true, now)
	c.put(traceID(4), true, now) // over the cap: evicts the stale slot's owner
